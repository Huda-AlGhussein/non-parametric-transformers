...
if c.exp_load_from_checkpoint:
    ...
    npt_model.load_state_dict(checkpoint['model_state_dict'])

test_loader = DataLoader(test_dataset, batch_size=c.exp_batch_size, shuffle=False, num_workers=2)

npt_model.eval()
predictions = []
targets = []

for batch in test_dataset.N:
    inputs, labels = batch
...
# For example, compute ROC AUC score
...
npt_model = NPTModel(c=c, metadata=test_dataset.metadata, device=device)
# Load model checkpoint
#npt_model.load_state_dict(torch.load(c.exp_checkpoint_path, map_location=device), strict=False)
#npt_model_torch_dtype = get_torch_dtype(dtype_name=c.model_dtype)
#npt_model = npt_model.to(device=device).type(npt_model_torch_dtype)
...
#optimizer = init_optimizer(c= c, model_parameters= npt_model.parameters(), device=device)
...
with torch.no_grad():
   # test.dataset.load_next_cv_split()
    ...
    #     outputs = npt_model(inputs)
    #     # Compute metrics like ROC AUC
    #     roc_auc = roc_auc_score(labels.cpu().numpy(), outputs.cpu().numpy())
    #     print(f'ROC AUC: {roc_auc}')
    for batch_index, batch_dict in enumerate(test_loader):
        ...
        #Concatenate the lists into single tensors (bcs of error: AttributeError: 'list' object has no attribute 'cpu')
        #ground_truth_tensors = torch.cat(ground_truth_tensors).cpu().numpy()
        #outputs = torch.cat(outputs).cpu().numpy()
        # target col (last column in our datasets)
        ...
        #print(true)
        # Compute metrics like ROC AUC
        #roc_auc = roc_auc_score(ground_truth_tensors, outputs)
